...
import com.luis.curso.springboot.error.models.Error;
...
public class HandlerExceptionController {
	...
	public Map<String, Object> numberFormatException(NumberFormatException e) {
		...
		error.put("error", "Numero incorrecto, no tiene formato digito");
	...
	public ResponseEntity<Error> notFound(NoHandlerFoundException e) {
		Error error = new Error();
		error.setDate(new Date());
		error.setError("Api Rest no contrada");
		error.setMessage(e.getMessage());
	...
	public Map<String, Object> userNotFoundException(Exception e) {
		Map<String, Object> error = new HashMap<>();
		error.put("date", new Date());
		error.put("error", "El usurio y/o rol no existe");
		error.put("message", e.getMessage());
		error.put("status", HttpStatus.INTERNAL_SERVER_ERROR.value());
		return error;
	}
